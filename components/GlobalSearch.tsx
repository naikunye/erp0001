...
import { Search, Package, ShoppingCart, History, X } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[15vh] px-4 backdrop-blur-md bg-black/60" onClick={() => setIsOpen(false)}>
      <div className="w-full max-w-2xl bg-[#0a0a0c] border border-white/10 rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
        ...
        <div className="max-h-[50vh] overflow-y-auto p-3 custom-scrollbar">
            {!query ? (
                ...
                <div className="space-y-4">
                    {products.length > 0 && (
                        <div>
                            <div className="px-3 py-1 text-[9px] font-black text-slate-600 uppercase tracking-widest">产品资产库</div>
                            {products.map(p => (
                                <div key={p.id} className="flex items-center gap-4 px-4 py-3 rounded-2xl hover:bg-indigo-600/10 cursor-default transition-all">
                    ...
                    {orders.length > 0 && (
                        <div>
                            <div className="px-3 py-1 text-[9px] font-black text-slate-600 uppercase tracking-widest">业务订单</div>
                            {orders.map(o => (
                                <div key={o.id} className="flex items-center gap-4 px-4 py-3 rounded-2xl hover:bg-violet-600/10 cursor-default transition-all">
                    ...
                    {products.length === 0 && orders.length === 0 && (
                        <div className="p-10 text-center text-slate-700 italic text-xs">未匹配到相关资产</div>
                    )}
                </div>
            )}
        </div>
        ...
        <div className="px-6 py-3 bg-white/2 border-t border-white/5 text-[9px] text-slate-600 flex justify-between items-center font-bold uppercase tracking-widest">
            <span>Tanxing OS Indexer</span>
            <div className="flex gap-4"><span>ESC 关闭</span></div>
        </div>
      </div>
    </div>
  );
