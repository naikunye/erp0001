...
const Sidebar: React.FC<SidebarProps> = ({ activePage, onNavigate, onLogout }) => {
  ...
  const menuItems = [
    ...
    { id: 'finance', label: '财务穿透', subLabel: 'Penetration', icon: Wallet },
  ...
  return (
    <>
        {isOpen && (
            <div className="fixed inset-0 bg-black/60 z-40 lg:hidden backdrop-blur-md" onClick={() => dispatch({ type: 'TOGGLE_MOBILE_MENU', payload: false })} />
        )}
                ...
                <nav className="flex-1 space-y-1 overflow-y-auto scrollbar-none px-1 relative z-10">
                    {menuItems.map((item) => {
                        ...
                        return (
                            ...
                            >
                                {isActive && (
                                    <div className="absolute inset-0 bg-gradient-to-r from-violet-600/20 to-transparent border-l-2 border-violet-500 opacity-100"></div>
                                )}
                                ...
                                <div className="flex flex-col items-start relative z-10">
                                    ...
                                    <span className={`text-[8px] mt-1 font-mono tracking-wide uppercase ${isActive ? 'text-violet-300/70' : 'text-slate-600 group-hover:text-slate-500'}`}>{item.subLabel}</span>
                                </div>
                                {isHighlight && (
                                    <div className={`ml-auto w-1.5 h-1.5 rounded-full animate-pulse shadow-[0_0_8px_#8b5cf6] ${isActive ? 'bg-white' : 'bg-violet-500'}`}></div>
                                )}
                            </button>
                        );
                    })}
