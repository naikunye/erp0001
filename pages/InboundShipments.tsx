    ...
    <div className="ios-glass-panel rounded-xl border border-white/10 shadow-sm flex flex-col h-[calc(100vh-8rem)] relative">
        ...
        <div className="flex-1 overflow-auto p-6 grid grid-cols-1 lg:grid-cols-2 gap-4">
            {state.inboundShipments.map(shipment => (
                <div 
                    key={shipment.id}
                    className="ios-glass-card p-5 hover:border-indigo-500/50 transition-all group relative cursor-pointer"
                ...
                >
                    ...
                    <div className="flex items-center gap-2 text-xs text-slate-400 mb-4 bg-white/5 p-2 rounded border border-white/5">
        ...
        {/* Create Modal */}
        {showCreateModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm bg-black/60" onClick={() => setShowCreateModal(false)}>
                <div className="ios-glass-panel w-full max-w-4xl h-[85vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="grid grid-cols-3 gap-6">
                            <div className="col-span-1 space-y-4">
                                <div>
                                    ...
                                    <input type="text" value={newShipmentName} onChange={e => setNewShipmentName(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded p-2 text-sm text-white" placeholder="e.g. 10月大促补货" />
                                </div>
                                <div>
                                    <label className="text-xs text-slate-400 block mb-1">发货仓库 (From)</label>
                                    <select value={sourceWH} onChange={e => setSourceWH(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded p-2 text-sm text-white">
                                        {WAREHOUSES.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}
                                    </select>
                                </div>
                                <div>
                                    <label className="text-xs text-slate-400 block mb-1">目的仓库 (To)</label>
                                    <select value={destWH} onChange={e => setDestWH(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded p-2 text-sm text-white">
                            ...
                            <div className="col-span-2 bg-white/5 border border-white/10 rounded-xl p-4 flex flex-col">
                                <div className="mb-4 relative">
                                    ...
                                    <input 
                                        ...
                                        className="w-full pl-9 pr-4 py-2 bg-black/40 border border-white/10 rounded-lg text-sm text-white focus:border-indigo-500 outline-none"
                                        placeholder="搜索 SKU 添加商品..." 
                                    />
                                    {skuSearch && (
                                        <div className="absolute top-full left-0 w-full bg-black border border-white/10 rounded-lg mt-1 z-10 max-h-48 overflow-y-auto shadow-xl">
                                            {state.products
                                                ...
                                                .map(p => (
                                                    <div 
                                                        key={p.id} 
                                                        className="p-2 hover:bg-white/10 cursor-pointer flex justify-between items-center text-xs"
                                ...
                                <div className="flex-1 overflow-y-auto space-y-2">
                                    {selectedProducts.map((item, idx) => (
                                        <div key={item.product.id} className="bg-black/20 border border-white/10 p-3 rounded-lg flex items-center gap-4">
                                            <div className="w-8 h-8 bg-black/40 rounded flex items-center justify-center text-xs font-bold text-slate-400">{idx + 1}</div>
                                            ...
                                            <div className="text-right">
                                                ...
                                                <input 
                                                    ...
                                                    className="w-20 bg-black/40 border border-white/10 rounded px-2 py-1 text-center text-sm text-white font-mono" 
                                                />
                                            </div>
        ...
        {/* Detail Modal */}
        {selectedShipment && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm bg-black/60" onClick={() => setSelectedShipment(null)}>
                <div className="ios-glass-panel w-full max-w-2xl rounded-2xl shadow-2xl p-6 animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="space-y-2 max-h-60 overflow-y-auto">
                            ...
                            {selectedShipment.items.map((item, idx) => (
                                <div key={idx} className="flex justify-between items-center p-3 bg-white/5 border border-white/5 rounded-lg text-sm">
                        ...
                        <div className="flex gap-3 pt-4 border-t border-white/10">
                            <button className="flex-1 py-2 bg-white/5 hover:bg-white/10 text-white rounded-lg text-sm flex items-center justify-center gap-2">
