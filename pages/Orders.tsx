                              ...
                              {activeOrders.filter(o => o.status === col.id).map(order => (
                                  <div key={order.id} draggable onDragStart={(e) => onDragStart(e, order.id)} onClick={() => handleOpenOrder(order)} className={`ios-glass-card p-3 cursor-grab active:cursor-grabbing hover:border-indigo-500/50 hover:bg-white/10 transition-all group relative ${draggedOrderId === order.id ? 'opacity-50 border-dashed border-slate-600' : ''}`}>
                                      ...
                                      <div className="absolute right-2 top-2 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
                                          ...
                                          <button onClick={(e) => { e.stopPropagation(); handleDeleteOrder(order.id); }} className="p-1.5 bg-white/5 hover:bg-red-500/20 hover:text-red-400 text-slate-400 rounded shadow-lg transition-colors" title="删除"><Trash2 className="w-3 h-3" /></button>
                                      </div>
                                  </div>
                              ))}
                          </div>
                ...
                <tr key={order.id} className="hover:bg-white/5 transition-colors group cursor-pointer" onClick={() => handleOpenOrder(order)}>
                    ...
                    <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                            <button onClick={(e) => { e.stopPropagation(); handleOpenOrder(order); }} className="text-xs font-medium text-slate-400 hover:text-white bg-white/5 hover:bg-indigo-600 px-3 py-1.5 rounded transition-all">查看</button>
                            <button onClick={(e) => { e.stopPropagation(); handleDeleteOrder(order.id); }} className="p-1.5 text-slate-500 hover:text-red-400 hover:bg-white/5 rounded transition-colors"><Trash2 className="w-4 h-4" /></button>
                        </div>
                    </td>
                </tr>
                ))}
                          ...
                          {rules.map(rule => (
                              <div key={rule.id} className="ios-glass-card p-4 flex justify-between items-center group hover:border-indigo-500/30 transition-all">
                                  ...
                                  <div className="flex items-center gap-4">
                                      ...
                                      <button className="p-2 text-slate-500 hover:text-white hover:bg-white/10 rounded"><PlayCircle className="w-5 h-5" /></button>
                                  </div>
                              </div>
                          ))}
                          <button className="w-full py-3 border-2 border-dashed border-white/10 rounded-xl text-slate-500 text-sm font-bold hover:border-indigo-500/50 hover:text-indigo-400 transition-all flex items-center justify-center gap-2">
