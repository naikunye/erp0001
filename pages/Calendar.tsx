...
const Calendar: React.FC = () => {
  ...
  const generateInventoryEvents = (): CalendarEvent[] => {
      ...
      state.products.forEach(product => {
          ...
          generated.push({
              ...
              title: `⚠️ ${product.sku} 预计断货`,
              date: stockoutDateStr,
              type: 'stockout',
              description: `当前库存: ${product.stock}, 日均消耗: ${product.dailyBurnRate}。建议立即补货。`,
              autoGenerated: true,
              sku: product.sku
          });
          ...
          if (daysUntilReorder > 0) {
             ...
             generated.push({
                 ...
                 title: `📝 ${product.sku} 建议下单`,
                 date: reorderDateStr,
                 type: 'product',
                 description: `交期: ${leadTime}天, 安全库存: ${safetyStockDays}天。`,
                 autoGenerated: true,
                 sku: product.sku
             });
          }
  ...
  useEffect(() => {
      ...
      const manualEvents: CalendarEvent[] = [
          { id: '1', title: 'CP-Q1M 补货到仓', date: `${y}-${String(m+1).padStart(2, '0')}-05`, type: 'logistics', description: '预计500件入库 US-WEST' },
          { id: '2', title: '新品发布会直播', date: `${y}-${String(m+1).padStart(2, '0')}-12`, type: 'marketing', description: 'TikTok Shop 专场' },
          { id: '3', title: '供应商付款日', date: `${y}-${String(m+1).padStart(2, '0')}-15`, type: 'finance', description: '支付深圳工厂Q3尾款' },
          { id: '4', title: 'K7500 键盘首发', date: `${y}-${String(m+1).padStart(2, '0')}-20`, type: 'product', description: '全渠道上线' },
          { id: '5', title: '黑色星期五备货', date: `${y}-${String(m+1).padStart(2, '0')}-28`, type: 'logistics', description: '截止入仓时间' },
      ];
  ...
  const renderMonthView = () => {
      ...
      // Calculate padding based on weekStartDay
      // if weekStartDay is 1 (Mon): Sun(0) -> 6 padding, Mon(1) -> 0 padding
  ...
  const renderWeekView = () => {
      ...
      // diff = how many days to subtract to get to startDay
      // e.g. Mon(1) start. Today Wed(3). diff = 3-1 = 2.
      // e.g. Mon(1) start. Today Sun(0). diff = 0-1 = -1 -> +7 = 6.
  ...
  return (
    <div className="h-[calc(100vh-6rem)] flex flex-col space-y-4">
      ...
      <div className="flex flex-wrap gap-4 px-4 py-2 bg-slate-950/30 rounded-xl border border-slate-800/50 justify-center text-xs text-slate-400">
          ...
          <div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-amber-500"></span> 备货</div>
          <div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-red-500"></span> 缺货</div>
          <div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-indigo-500 animate-pulse"></span> AI 建议</div>
      </div>
