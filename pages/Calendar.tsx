...
const Calendar: React.FC = () => {
  ...
  const generateLogisticsEvents = (): CalendarEvent[] => {
      ...
      state.shipments.forEach(shipment => {
          if (shipment.estimatedDelivery && shipment.status !== 'Delivered') {
              ...
              events.push({
                  ...
                  title: `🚢 ${shipment.carrier} 到港 (${shipment.trackingNo.slice(-4)})`,
                  date: shipment.estimatedDelivery, // Ensure this matches format
                  type: 'logistics',
                  description: `${shipment.productName} 预计到达. Status: ${shipment.status}`,
                  autoGenerated: true
              });
          }
      });
  ...
  useEffect(() => {
      ...
      setEvents(prev => {
          // Keep manually added AI events
          const existingAiEvents = prev.filter(e => e.id.startsWith('AI-GEN-') || (!e.autoGenerated && !e.id.startsWith('AUTO-') && !e.id.startsWith('LOG-') && !e.id.startsWith('FIN-')));
          return [...existingAiEvents, ...inventoryEvents, ...logisticsEvents, ...financeEvents];
      });
  }, [state.products, state.shipments, state.transactions]); 
