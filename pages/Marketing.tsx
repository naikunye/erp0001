    ...
    return (
        <div className="h-full flex flex-col gap-6 animate-in fade-in slide-in-from-right-4">
            ...
            <div className="grid grid-cols-4 gap-4">
                <div className="flow-border-box p-4 rounded-xl">
                ...
                <div className="holo-card p-4 rounded-xl flex flex-col justify-center items-center hover:bg-white/5 transition-all cursor-pointer border border-dashed border-slate-700 hover:border-pink-500">
            ...
            <div className="flex-1 overflow-x-auto overflow-y-hidden">
                <div className="flex gap-4 h-full min-w-[1000px]">
                    {stages.map(stage => (
                        <div key={stage.id} className="flex-1 flex flex-col holo-card rounded-xl min-w-[280px]">
                            <div className={`p-3 border-b border-white/5 bg-slate-900/30 rounded-t-xl flex justify-between items-center backdrop-blur-sm ${stage.id === 'Video Live' ? 'bg-pink-900/20' : ''}`}>
                            ...
                            <div className="flex-1 overflow-y-auto p-3 space-y-3">
                                {influencers.filter(i => i.status === stage.id).map(inf => (
                                    <div key={inf.id} className="bg-slate-900/80 border border-white/5 rounded-lg p-3 hover:border-pink-500/50 hover:bg-slate-800/80 transition-all group relative shadow-lg hover:shadow-pink-500/10">
                                        <div className="flex items-start gap-3 mb-2">
                                            ...
                                            <div className="text-[10px] bg-black/40 border border-white/5 px-1.5 py-0.5 rounded text-slate-400">{inf.country}</div>
                                        </div>
                                        ...
                                        {inf.status === 'Sample Sent' && (
                                            <div className="mt-2 text-xs bg-black/30 p-2 rounded border border-white/5">
                                        ...
                                        {(inf.status === 'Video Live' || inf.status === 'Completed') && (
                                            <div className="mt-2 text-xs">
                                                <div className="grid grid-cols-2 gap-1 bg-black/30 p-2 rounded border border-white/5">
            ...
            {/* AI Outreach Modal */}
            {showOutreachModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm bg-black/70" onClick={() => setShowOutreachModal(null)}>
                    <div className="bg-slate-900 w-full max-w-2xl h-[80vh] rounded-2xl border border-slate-800 shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 hud-card" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="flex-1 flex p-6 gap-6 overflow-hidden">
                            {/* Profile */}
                            <div className="w-1/3 border-r border-slate-800 pr-6 space-y-4">
                            ...
                            <div className="flex-1 flex flex-col">
                                ...
                                <textarea 
                                    ...
                                    className="flex-1 bg-slate-950/50 border border-slate-800 rounded-xl p-4 text-sm text-slate-300 focus:outline-none focus:border-indigo-500 resize-none font-mono leading-relaxed"
                                    placeholder="正在生成..."
                                />
            ...
            {showAddModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm bg-black/70" onClick={() => setShowAddModal(false)}>
                    {/* ... Existing Add Modal Code ... */}
                    <div className="bg-slate-900 w-full max-w-md rounded-2xl border border-slate-800 shadow-2xl p-6 hud-card" onClick={e => e.stopPropagation()}>
                        <h3 className="text-lg font-bold text-white mb-4">添加红人</h3>
                        {/* Inputs ... */}
                        <div className="space-y-3">
                            <input type="text" placeholder="昵称" value={newInf.name} onChange={e => setNewInf({...newInf, name: e.target.value})} className="w-full bg-slate-950 border border-slate-700 rounded p-2 text-sm text-white" />
                            <input type="text" placeholder="账号" value={newInf.handle} onChange={e => setNewInf({...newInf, handle: e.target.value})} className="w-full bg-slate-950 border border-slate-700 rounded p-2 text-sm text-white" />
            ...
            {showSampleModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm bg-black/70" onClick={() => setShowSampleModal(null)}>
                    <div className="bg-slate-900 w-full max-w-sm rounded-2xl border border-slate-800 p-6 hud-card" onClick={e => e.stopPropagation()}>
                        <h3 className="text-lg font-bold text-white mb-4">寄样登记</h3>
                        <div className="space-y-4">
                            <select value={sampleForm.sku} onChange={e => setSampleForm({...sampleForm, sku: e.target.value})} className="w-full bg-slate-950 border border-slate-700 rounded p-2 text-sm text-white"><option value="">Select SKU...</option>{products.map(p => <option key={p.id} value={p.sku}>{p.sku}</option>)}</select>
...
const AdCommandCenter = () => { const { state } = useTanxing(); return <div className="p-4 text-center text-slate-500 holo-card">Ads Module Active ({state.adCampaigns.length} campaigns)</div>; };
...
const ContentEngine = () => {
    ...
    return (
        <div className="h-full grid grid-cols-1 lg:grid-cols-3 gap-6 p-2 animate-in fade-in slide-in-from-bottom-2">
            ...
            <div className="lg:col-span-1 flow-border-box p-5 flex flex-col gap-5 shadow-lg">
                <div className="relative z-10 flex flex-col h-full">
                    ...
                    <div className="space-y-4 flex-1">
                        <div>
                            ...
                            <input type="number" value={sellingPrice} onChange={e => setSellingPrice(Number(e.target.value))} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2.5 text-white text-sm focus:border-indigo-500 outline-none" />
                        </div>
                        
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="text-xs text-slate-400 block mb-1">采购成本 ($)</label>
                                <input type="number" value={productCost} onChange={e => setProductCost(Number(e.target.value))} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2.5 text-white text-sm focus:border-indigo-500 outline-none" />
                            </div>
                            <div>
                                <label className="text-xs text-slate-400 block mb-1">物流履约 ($)</label>
                                <input type="number" value={logisticsCost} onChange={e => setLogisticsCost(Number(e.target.value))} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2.5 text-white text-sm focus:border-indigo-500 outline-none" />
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" value={platformFeePercent} onChange={e => setPlatformFeePercent(Number(e.target.value))} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2.5 text-white text-sm focus:border-indigo-500 outline-none" />
                            </div>
                            <div>
                                <label className="text-xs text-slate-400 block mb-1">广告 CPA ($)</label>
                                <input type="number" value={adSpend} onChange={e => setAdSpend(Number(e.target.value))} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2.5 text-white text-sm focus:border-indigo-500 outline-none" />
                            </div>
                        </div>
                    </div>
            ...
            <div className="lg:col-span-2 holo-card p-6 relative overflow-hidden flex flex-col shadow-lg scanline-overlay">
...
const Marketing: React.FC = () => {
  ...
  return (
    <div className="h-[calc(100vh-6rem)] flex flex-col gap-6 p-2">
        {/* Glass Tab Switcher */}
        <div className="flex bg-slate-900/80 backdrop-blur-md p-1 rounded-xl border border-white/10 w-fit shadow-lg">
