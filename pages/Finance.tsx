  ...
  const penetrationData = useMemo(() => {
      ...
      return {
          ...
          breakdown: [
              ...
              { name: '预估退货及损耗', value: -(fixedFee + refundRateLoss), color: '#94a3b8' },
              { name: '穿透式净利', value: netProfit, color: '#8b5cf6' }
          ],
    ...
    <div className="space-y-6 h-full flex flex-col">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shrink-0">
          <div>
              ...
              <p className="text-[10px] text-slate-500 font-mono mt-1 uppercase tracking-widest">Logic: Normalized Allocation V3.6 • Sync Stable</p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 shrink-0">
          <div className="ios-glass-card p-4 border-l-4 border-l-blue-500"><div className="text-[9px] text-slate-500 font-bold uppercase mb-1">现金流余额</div><div className="text-xl font-black text-white font-mono">¥ {financeStats.balance.toLocaleString()}</div></div>
          <div className="ios-glass-card p-4 border-l-4 border-l-emerald-500"><div className="text-[9px] text-slate-500 font-bold uppercase mb-1">库存采购货值</div><div className="text-xl font-black text-emerald-400 font-mono">¥ {state.products.reduce((a,b)=>a+(b.stock*(b.costPrice||0)),0).toLocaleString()}</div></div>
          <div className="ios-glass-card p-4 border-l-4 border-l-rose-500"><div className="text-[9px] text-slate-500 font-bold uppercase mb-1">本月支出合计</div><div className="text-xl font-black text-rose-400 font-mono">¥ {financeStats.expense.toLocaleString()}</div></div>
          <div className="ios-glass-card p-4 border-l-4 border-l-amber-500"><div className="text-[9px] text-slate-500 font-bold uppercase mb-1">待核销/待收款</div><div className="text-xl font-black text-amber-500 font-mono">¥ {financeStats.pending.toLocaleString()}</div></div>
      </div>
              ...
              <div className="h-full flex flex-col ios-glass-panel rounded-2xl overflow-hidden animate-in fade-in">
                  <div className="p-4 bg-white/5 border-b border-white/5 flex justify-between items-center">
                      <div className="relative">
                          ...
                          <input type="text" placeholder="检索交易流水..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="bg-black/40 border border-white/10 rounded-lg pl-9 pr-4 py-2 text-xs text-white focus:border-indigo-500 outline-none w-64"/>
                      </div>
                  </div>
                  <div className="flex-1 overflow-y-auto custom-scrollbar">
                      <table className="w-full text-left">
                          <thead className="bg-black/40 sticky top-0 z-10 border-b border-white/5 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                              <tr><th className="p-4">结算日期</th><th className="p-4">分类</th><th className="p-4">交易描述</th><th className="p-4 text-right">交易金额</th><th className="p-4 text-center">状态</th></tr>
                          </thead>
          ...
          {viewMode === 'penetration' && penetrationData && (
              <div className="h-full grid grid-cols-12 gap-6 animate-in slide-in-from-right-4">
                   <div className="col-span-4 ios-glass-card p-6 overflow-y-auto">
                        <h4 className="text-xs font-bold text-slate-500 uppercase mb-4 tracking-tighter">选择分析 SKU</h4>
                        <div className="space-y-2">
                            {state.products.filter(p => !p.deletedAt).map(p => (
                                <div key={p.id} onClick={() => setSelectedSku(p.sku)} className={`p-4 rounded-xl border cursor-pointer transition-all ${selectedSku === p.sku ? 'bg-violet-600/10 border-violet-500 shadow-inner' : 'bg-white/5 border-white/5 hover:border-white/10'}`}>
                   ...
                   <div className="col-span-8 ios-glass-card p-10 flex flex-col relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-end mb-10 relative z-10">
                            <div>
                                <h3 className="text-xl font-bold text-white italic uppercase tracking-tighter mb-1">单品穿透损益瀑布模型 (P&L Waterfall)</h3>
                                <p className="text-xs text-slate-500 font-mono">Real-time Marginal Analysis | ER 1:{EXCHANGE_RATE}</p>
                            </div>
                            <div className="text-right">
                                <div className="text-[10px] text-slate-500 font-bold uppercase mb-1">库存预期净利 (USD)</div>
                                <div className={`text-4xl font-black font-mono tracking-tighter ${penetrationData.totalPotentialProfit >= 0 ? 'text-white' : 'text-rose-500 animate-pulse'}`}>
                        ...
                        <div className="mt-8 p-6 bg-violet-600/10 border border-violet-500/30 rounded-2xl flex justify-between items-center relative overflow-hidden">
                            <div className="flex items-center gap-4 relative z-10">
                                ...
                                <div>
                                    <div className="text-xs font-bold text-white uppercase tracking-widest">综合穿透净利率 (Net Margin)</div>
                                    <div className="text-[10px] text-violet-400 font-mono mt-0.5">计入退货、营销、物流及平台损耗</div>
                                </div>
                            </div>
