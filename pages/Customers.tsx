    ...
    <div className="bg-slate-900 rounded-xl border border-slate-800 shadow-sm flex flex-col h-[calc(100vh-8rem)] relative">
      ...
      <div className="p-4 border-b border-slate-800 flex flex-col sm:flex-row justify-between items-center gap-4 bg-slate-900/50">
        <div className="flex items-center gap-2">
            ...
            <span className="px-2 py-0.5 bg-slate-800 text-slate-400 text-xs rounded border border-slate-700">{customers.length}</span>
        </div>
        ...
        <div className="flex space-x-3 w-full sm:w-auto">
            <div className="relative">
                <input 
                    ...
                    className="w-64 pl-9 pr-4 py-1.5 bg-slate-950 border border-slate-700 rounded-lg text-sm text-slate-300 focus:outline-none focus:ring-1 focus:ring-indigo-500 transition-all placeholder-slate-600"
                />
            ...
            <div className="relative">
                <button 
                    ...
                    className={`flex items-center justify-center space-x-2 px-3 py-1.5 border rounded-lg transition-colors text-sm ${filterSource !== 'All' ? 'bg-indigo-600 border-indigo-500 text-white' : 'border-slate-700 text-slate-400 hover:bg-slate-800 hover:text-white'}`}
                >
                ...
                {/* Filter Dropdown */}
                {showFilter && (
                    <div className="absolute top-full right-0 mt-2 w-40 bg-slate-900 border border-slate-800 rounded-lg shadow-xl z-20 animate-in fade-in zoom-in-95 overflow-hidden">
                        <div className="text-[10px] uppercase font-bold text-slate-500 px-3 py-2 bg-slate-950/50 border-b border-slate-800">来源筛选</div>
                        {['All', 'Offline', 'TikTok', 'Amazon', 'Shopify'].map(source => (
                            <button
                                key={source}
                                onClick={() => { setFilterSource(source); setShowFilter(false); }}
                                className={`w-full text-left px-3 py-2 text-xs hover:bg-slate-800 transition-colors flex items-center justify-between ${filterSource === source ? 'text-indigo-400 bg-slate-800/50' : 'text-slate-300'}`}
                            >
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-800/50">
                {filteredCustomers.map(customer => (
                    <tr key={customer.id} className="hover:bg-slate-800/30 transition-colors group cursor-pointer" onClick={() => handleEditClick(customer)}>
                        ...
                        <td className="px-6 py-4">
                            <div className="flex flex-col gap-1.5">
                                <span className="text-xs text-slate-300 bg-slate-800 border border-slate-700 px-2 py-0.5 rounded w-fit">{customer.source}</span>
      ...
      {selectedCustomer && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm bg-black/60" onClick={closeModal}>
              <div className="bg-slate-900 w-full max-w-2xl rounded-2xl border border-slate-800 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                  ...
                  <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
                              ...
                              <div className="space-y-1">
                                  ...
                                  <div className="flex gap-2">
                                      <button 
                                          ...
                                          className={`flex-1 py-2 text-xs rounded-lg border ${selectedCustomer.status === 'Active' ? 'bg-emerald-500/20 border-emerald-500 text-emerald-400' : 'bg-slate-950 border-slate-700 text-slate-500'}`}
                                      >
                                          活跃 (Active)
                                      </button>
                                      <button 
                                          onClick={() => updateField('status', 'Inactive')}
                                          className={`flex-1 py-2 text-xs rounded-lg border ${selectedCustomer.status === 'Inactive' ? 'bg-slate-700 border-slate-500 text-white' : 'bg-slate-950 border-slate-700 text-slate-500'}`}
                                      >
                                          停用 (Inactive)
                                      </button>
                                  </div>
                      ...
                      <div className="space-y-2 pt-2 border-t border-slate-800">
                           ...
                           <textarea 
                              ...
                              className="w-full h-24 px-3 py-2 bg-slate-950 border border-slate-700 rounded-lg text-sm text-white focus:outline-none focus:border-indigo-500 resize-none"
