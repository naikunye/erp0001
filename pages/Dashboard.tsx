...
import { 
    ...
    ChevronRight, ArrowRight, MessageSquare, Bell, Landmark
} from 'lucide-react';
...
const Dashboard: React.FC = () => {
  ...
  const metrics = useMemo(() => {
      ...
      let completedIncome = 0; let completedExpense = 0; let pendingLiabilities = 0;
      (state.transactions || []).forEach(t => {
          const val = t.currency === 'USD' ? t.amount * EXCHANGE_RATE : t.amount;
          if (t.type === 'income') { if (t.status === 'completed') completedIncome += val; } 
          else { if (t.status === 'completed') completedExpense += val; else if (t.status === 'pending') pendingLiabilities += val; }
      });
      ...
      return { 
          ...
          lowStockSkus: activeProducts.filter(p => {
              ...
              return p.stock / daily < (p.safetyStockDays || 7);
          }).length 
      };
  ...
  return (
    <div className="space-y-6 pb-10 animate-in fade-in duration-700 slide-in-from-bottom-4">
      ...
      <div className="ios-glass-panel border-indigo-500/20 bg-indigo-950/20 h-16 px-5 rounded-2xl flex items-center gap-6 relative overflow-hidden group">
          ...
          {/* Fix: Added dispatch to handle navigation to automation page */}
          <button 
      ...
      {/* 核心 KPI 矩阵 (重构为 2行 x 3列 的平铺网格，防止排版重叠) */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <StatCard 
                loading={isLoading} 
                title="预估总净利" 
                value={`$${metrics.totalPotentialProfitUSD.toLocaleString(undefined, {maximumFractionDigits:3})}`} 
